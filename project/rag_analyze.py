...
_FALLBACK: Dict[str, Any] = {
    ...
    "difficulty": "Unknown",
    "message":    "Video processed. Ask me anything about it.",
    "labels": {
        "summary": "Short Summary",
        "main_topic": "Main Topic",
        "motive": "Video Motive",
        "teaching": "Teaching Goals",
...
class YouTubeRAG:
        ...
        messages = [
            {
                ...
                "content": (
                    ...
                    "synonyms, and context words for the user's query. Output ONLY a space-separated list of "
    ...
    def _ensure_english(self, text: str) -> str:
        """
        Check if the text is primarily in English. If it contains significant
        non-English scripts, call the LLM to translate it to English.
        """
        # Heuristic: check for non-ASCII characters (common for Hindi/Devanagari scripts)
        # If more than 5% of characters are non-ASCII, we treat it as non-English.
        non_ascii = len([c for c in text if ord(c) > 127])
        if non_ascii > (len(text) * 0.05):
            logger.info("Non-English output detected, translating automatically...")
            messages = [
                {
                    "role": "system",
                    "content": "You are a professional translator. Translate the following text into clear, simple English. Output ONLY the translation."
                },
    ...
    def ask(self, question: str) -> str:
        ...
        messages = [
            {
                "role": "system",
                "content": (
                    "You are a professional, smart, and direct AI assistant. STRICT RULES:\n"
                    "1. LANGUAGE: Respond ONLY in clear, professional English. If the transcript is non-English, translate the context silently.\n"
                    "2. LENGTH CONTROL:\n"
                    "   - Simple queries -> Short answers (2-4 sentences max).\n"
                    "   - Concept/Explanation queries -> Medium length.\n"
                    "   - If user asks 'explain', 'in detail', or 'elaborate' -> Detailed long answers.\n"
                    "3. CODE RULE: For code-related tasks or requests, return ONLY the code block. No explanation unless explicitly asked.\n"
                    "4. NO VERBOSITY: Eliminate report-style or overly descriptive filler. Avoid 'Based on the video...', 'This video explains...'. Start directly.\n"
                    "5. STYLE: Be clean, user-friendly, and concise. Use ALL CAPS only for critical emphasis, not every important word.\n"
                    "6. FAILSAFE: If the exact answer isn't in context, use broader knowledge to guess accurately. NEVER say 'no information found'."
                ),
            },
            {
                "role": "user",
        ...
        raw_answer = self._llm(messages)
        return self._ensure_english(raw_answer)
    ...
    def generate_analysis(self, title: str, channel: str, transcript: str) -> Dict[str, Any]:
        ...
        prompt = (
            ...
            '"message":"A greeting in English (e.g. \"I have finished the analysis! Ask me anything.\")",'
            '"labels":{"summary":"Short Summary", "main_topic":"Main Topic", "motive":"Video Motive", "teaching":"Teaching Goal", "key_points":"Key Points", "learning":"Learning Outcomes", "difficulty":"Difficulty"} (All in English)}\n\n'
            f"Transcript:\n{truncated}"
        )
        messages = [
            {
        ...
        try:
            ...
            # Ensure the values in the JSON are in English (the keys already are)
            # We don't want to translate the whole JSON string as it might break format,
            # but the _ensure_english check on the raw string is a good fallback.
            # However, for JSON, a better rule is to just be extremely strict in prompt.
            raw = self._ensure_english(raw)
